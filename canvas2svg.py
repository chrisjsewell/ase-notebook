...
# WWW   : http://0x80.pl/
# Home  : https://github.com/WojciechMula/canvas2svg
...
WARNINGS_MODULE = 200
WARNINGS_NONE = 300
WARNINGS_MODE = WARNINGS_PYTHON

SEGMENT_TO_LINE = 1000
SEGMENT_TO_PATH = 2000


def set_warnings_mode(mode):
    """Configure how warnings are emmited."""
    global WARNINGS_MODE

    if mode not in [WARNINGS_PYTHON, WARNINGS_MODULE, WARNINGS_NONE]:
...
def emit_warning(msg):
    """Emit a warning, in the given mode."""
    if WARNINGS_MODE == WARNINGS_PYTHON:
        warn(msg)
    elif WARNINGS_MODE == WARNINGS_MODULE:
...
def set_segment_func(flag):
    """Configure whether to use either <path> or <line> tag for segment representation.

    - ``SEGMENT_TO_LINE``: use <line> tag (default)
    - ``SEGMENT_TO_PATH``: use <path> tag.

    """
    global SEGMENT_FUNC

    if flag == SEGMENT_TO_LINE:
        SEGMENT_FUNC = segment_to_line
    elif flag == SEGMENT_TO_PATH:
        SEGMENT_FUNC = segment_to_path
    else:
        raise ValueError(
...
def convert_canvas(document, canvas, items=None):
    ...
    ----------
    document: object
        object that supports DOM, i.e. provides createElement method etc.
        (function initialise_svg_document can be used to get such an object)
    ...
    for item in items:
        ...
        options = {v0: v4 for v0, v1, v2, v3, v4 in tmp.values()}

        # get state of item
        state = options["state"]
        if "current" in options["tags"]:
            options["state"] = const.ACTIVE
        elif options["state"] == "":
            options["state"] = "normal"
        else:
            # left state unchanged
            assert options["state"] in ["normal", const.DISABLED, "hidden"]
        ...
        def get(name, default=""):
            if state == const.ACTIVE and options.get(state + name):
                return options.get(state + name)
            if state == const.DISABLED and options.get(state + name):
                return options.get(state + name)
        ...
        elif itemtype == "arc" and options["style"] == const.ARC:
            options["fill"] = ""
            options["activefill"] = ""
            options["disabledfill"] = ""

        style = {}
        style["stroke"] = get_html_color(canvas, get("outline"))

        if get("fill"):
            style["fill"] = get_html_color(canvas, get("fill"))
        else:
            style["fill"] = "none"
        ...
        if width:
            ...
            if state == const.DISABLED and canvas.itemcget(item, "disableddash"):
                dash = canvas.itemcget(item, "disableddash")
            elif state == const.ACTIVE and canvas.itemcget(item, "activedash"):
        ...
        if itemtype == "line":
            ...
            if options["smooth"] in ["1", "bezier", "true"]:
                ...
                if len(coords) == 4:
                    # segment
                    element = SEGMENT_FUNC(document, coords)
                else:
                    # polyline
                ...
                emit_warning(
                    "Unknown smooth type: %s. Falling back to smooth=0"
                    % options["smooth"]
                )
            ...
            if options["arrow"] in [const.FIRST, const.BOTH]:
                arrow = arrow_head(
                    document,
                    coords[2],
                    coords[3],
            ...
            if options["arrow"] in [const.LAST, const.BOTH]:
                arrow = arrow_head(
                    document,
                    coords[-4],
                    coords[-3],
...
def initialise_svg_document():
    """Initialise default SVG document."""
...
def segment_to_line(document, coords):
    """Create polyline with 2 vertices using <line> tag."""
    return setattribs(
...
def segment_to_path(document, coords):
    """Create polyline with 2 vertices using <path> tag."""
    return setattribs(
        document.createElement("path"), d="M%s,%s %s,%s" % tuple(coords[:4])
    )
...
def polyline(document, coords):
    """Create polyline with more then 2 vertices."""
    points = []
...
def smoothpolygon(document, coords):
    """Create smoothed, filled polygon."""
    element = document.createElement("path")
    path = []
    points = [(coords[i], coords[i + 1]) for i in range(0, len(coords), 2)]
...
def arc(document, bounding_rect, start, extent, style):
    """Create arc, pieslice (filled), arc with chord (filled)."""
...
def get_html_color(canvas, color):
    """Return Tk color in form '#rrggbb' or '#rgb'."""
    if color:
        # r, g, b \in [0..2**16]
...
def arrow_head(document, x0, y0, x1, y1, arrowshape):
    ...
    xa, ya = lerp(point1, point0, d1 / d)
    xb, yb = lerp(point1, point0, d2 / d)
...
def font_actual(tkapp, font):
    """Return actual font parameters."""
    tmp = tkapp.call("font", "actual", font)
    return {tmp[i][1:]: tmp[i + 1] for i in range(0, len(tmp), 2)}


def font_metrics(tkapp, font, font_property=None):
    """Return font metrics."""
    if font_property is None:
        tmp = tkapp.call("font", "metrics", font)
        return {tmp[i][1:]: int(tmp[i + 1]) for i in range(0, len(tmp), 2)}
    else:
        return int(tkapp.call("font", "metrics", font, "-" + font_property))
...
JOIN_STYLE = {"bevel": "bevel", "miter": "", "round": "round"}  # SVG default

TEXT_ANCHOR = {
    const.SE: "end",
    const.E: "end",
    const.NE: "end",
    const.SW: "",  # SVG defaul (value "start")
    const.W: "",
    const.NW: "",
    const.N: "middle",
    const.S: "middle",
...
FONT_STYLE = {"italic": "italic", "roman": ""}  # SVG default

FONT_WEIGHT = {"bold": "bold", "normal": ""}  # SVG default
